...
import { loadState } from "viem/_types/actions/test/loadState";
...
describe("Ballot", async () => {
  ...
  return { publicClient, deployer, otherAccount, ballotContract };
    ...
    it("sets the deployer address as chairperson", async () => {
        ...
        expect(deployer.account.address).to.be.equal(chairperson.toLowerCase());

    });
    it("sets the voting weight for the chairperson as 1", async () => {
      const {ballotContract,deployer}= await loadFixture(deployContract);
      const chairperson= await ballotContract.read.voters([deployer.account.address]);
      expect(chairperson[0]).to.be.equal(1n);
    });
  });
